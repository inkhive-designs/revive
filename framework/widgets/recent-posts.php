...
class revive_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'revive_rp', // Base ID
			__('Revive Recent Posts with Thumbnails', 'revive'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'revive' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		<?php
         ...
         ?>
		         ...
		         <div class='rp-thumb'><a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('thumbnail',array(  'alt' => get_the_title(),'title' => get_the_title() )); ?></a></div>
		         <?php 
		         else :
		         ?>
		...
		?>
		      ...
		      <?php esc_html_e('Oops, there are no posts.','revive') ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'revive' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'revive' );
		}
		?>
		<p>
...
add_action( 'widgets_init', 'register_revive_widget' );  
function register_revive_widget() {  
    register_widget( 'revive_Recent_Posts' );  
}  
